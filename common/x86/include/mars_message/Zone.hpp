...
#ifndef __mars_message_Zone_hpp__
#define __mars_message_Zone_hpp__
...
class Zone
{
    ...
    public:
        NLOHMANN_DEFINE_TYPE_INTRUSIVE(Zone, count, polygon, name)
#endif
#ifdef __MSGPACK
    public:
        MSGPACK_DEFINE(Zone, count, polygon, name)
#endif
    public:
        bool operator==(const Zone& other) const
        {
            return count == other.count && polygon == other.polygon && name == other.name;
        }
    public:
        /**
...
int Zone::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int Zone::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int Zone::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t Zone::getHash()
{
    static int64_t hash = static_cast<int64_t>(_computeHash(NULL));
    return hash;
}

const char* Zone::getTypeName()
{
    return "Zone";
}

int Zone::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int Zone::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int Zone::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t Zone::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == Zone::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, Zone::getHash };

    uint64_t hash = 0x4803f3bac9efd03aLL +
