...
#ifndef __mars_message_GridMap_hpp__
#define __mars_message_GridMap_hpp__
...
class GridMap
{
    ...
    public:
        NLOHMANN_DEFINE_TYPE_INTRUSIVE(GridMap, timestampMs, mapLoadTime, resolution, width, height, origin, pointCount, data)
#endif
#ifdef __MSGPACK
    public:
        MSGPACK_DEFINE(GridMap, timestampMs, mapLoadTime, resolution, width, height, origin, pointCount, data)
#endif
    public:
        bool operator==(const GridMap& other) const
        {
            return timestampMs == other.timestampMs && mapLoadTime == other.mapLoadTime && resolution == other.resolution && width == other.width && height == other.height && origin == other.origin && pointCount == other.pointCount && data == other.data;
        }
    public:
        /**
...
int GridMap::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &this->mapLoadTime, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int GridMap::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int64_t_decode_array(buf, offset + pos, maxlen - pos, &this->mapLoadTime, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
uint64_t GridMap::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == GridMap::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, GridMap::getHash };

    uint64_t hash = 0xb1198c74f267aebbLL +
